...
- **Changelog pre-processing**: `_generateProjectData()` runs the raw CHANGELOG.md through `MarkdownProcessor` + `OpalHighlighter` at generation time, so `project_data.dart` stores highlighted HTML (not raw markdown). The `ChangelogPage` template renders it with `RawText` via `changelog.raw`, not the `Markdown` component. TOC entries and HTML are wrapped in a `Changelog(raw: ..., toc: [...])` object — the generated `ChangelogPage` accesses `context.project.changelog?.raw` and `context.project.changelog?.toc`.
