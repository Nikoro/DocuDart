...
Config configure(Project project) => Config(
  ...
  header: () => Header(
    leading: Logo(
      image: img(src: Assets.logo.logo_webp, alt: '${project.pubspec.name} logo'),
      title: project.pubspec.name,
    ),
    links: [
      ...
      ?project.pubspec.repository.let(
        (repository) =>
            .url(repository.link, label: repository.label, leading: repository.icon, trailing: Icons.openInNew),
      ),
