---
name: regenerate
description: Removes the website/ directory in a target folder and re-runs docudart init --full to regenerate it. Accepts an optional directory argument (defaults to "example").
---
...
4. Run `docudart init --full` from the target directory:
   ```
   cd <target> && dart run ../bin/docudart.dart init --full
   ```
